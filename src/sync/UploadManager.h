...
struct LocalGameInfo {
    ...
    bool needsUpload;         // true if local but not on cloud
};
...
public:
    ...
    // Start reverse sync (upload local games not on cloud)
...
private:
    ...
    std::string findDriveFolderForSystem(const std::string& systemCode);
